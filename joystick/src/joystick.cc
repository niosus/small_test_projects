...
void cJoystick::readEv()
{
    ...
    if (bytes > 0)
    {
        ...
        {
            cerr << "button pressed " << (int) joystick_ev->number << " !!! " << joystick_ev->value << endl;
            joystick_st->button[joystick_ev->number] = joystick_ev->value;
        }
        if (joystick_ev->type & JS_EVENT_AXIS)
        {
        	cerr << "axis used " << (int) joystick_ev->number << " !!! " << joystick_ev->value << endl;
            joystick_st->axis[joystick_ev->number] = joystick_ev->value;
        }
    }
}

joystick_position cJoystick::joystickPosition(int n1, int n2)
{
    joystick_position pos;
    if (n1 > -1 && n1 < axes && n2 > -1 && n2 < axes)
    {
        int i0 = n1, i1 = n2;
